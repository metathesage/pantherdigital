...
const UA = { "User-Agent": "PNHRDGTL/1.0", Accept: "application/json" } as const;
const TTL_MS = 60_000;
...
function normalizeDsPair(pair: DsPair, boost: DsBoost, profile: DsProfile, feed: Feed): UnifiedPair | null {
...
async function fetchBoostsFeed(chain: ChainQ): Promise<{ pairs: UnifiedPair[]; degraded: boolean }> {
  ...
  const hydrated = await mapPool(uniq, 6, async (b) => {
    ...
    return normalizeDsPair(best, b, profile as DsProfile, "boosts");
  });
