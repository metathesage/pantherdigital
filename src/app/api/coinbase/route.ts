...
// PNTHR DGTL — Coinbase Advanced Trade proxy. Public market data is keyless;
// private endpoints need an API key + secret. The UUID key
// (COINBASE_API_KEY) authenticates PRIVATE endpoints only when paired with
// the API SECRET. Public market-data endpoints work without any auth, so
// this proxy serves both. Key stays server-side via process.env.
...
export async function GET(req: Request) {
  ...
  try {
    const headers: Record<string, string> = { "Content-Type": "application/json", "User-Agent": "PNHRDGTL/1.0" };
