...
async function coinLoreFallback(id: string): Promise<unknown | null> {
  ...
  try {
    ...
    const [info, ticker, social, markets] = await Promise.all([
      fetch(`${base}/coin/info/?id=${cid}`, { cache: "no-store", headers: { Accept: "application/json" } })
        .then((r) => (r.ok ? r.json() : null)).catch(() => null),
      fetch(`${base}/ticker/?id=${cid}`, { cache: "no-store", headers: { Accept: "application/json" } })
        .then((r) => (r.ok ? r.json() : null)).catch(() => null),
      fetch(`${base}/coin/social_stats/?id=${cid}`, { cache: "no-store", headers: { Accept: "application/json" } })
        .then((r) => (r.ok ? r.json() : null)).catch(() => null),
      fetch(`${base}/coin/markets/?id=${cid}`, { cache: "no-store", headers: { Accept: "application/json" } })
        .then((r) => (r.ok ? r.json() : null)).catch(() => null),
    ]);
    ...
    const csupply = tick?.csupply != null ? num(tick.csupply) : (info0?.csupply != null ? num(info0.csupply) : null);
    const tsupply = tick?.tsupply != null ? num(tick.tsupply) : (info0?.tsupply != null ? num(info0.tsupply) : null);
    ...
    const socials = (social ?? {}) as { reddit?: { subscribers?: unknown }; twitter?: { followers_count?: unknown } };
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (id.startsWith("cl-")) {
    ...
    if (fb) {
      return NextResponse.json(fb, { headers: { "Cache-Control": "public, s-maxage=300, stale-while-revalidate=600", "X-CP-Source": "coinlore" } });
    }
  ...
  try {
    ...
    if (!r.ok) {
      // CoinGecko rate-limited/404 — fall back to CoinLore detail
      const fb = await coinLoreFallback(id);
      if (fb) {
        return NextResponse.json(fb, { headers: { "Cache-Control": "public, s-maxage=300, stale-while-revalidate=600", "X-CP-Source": "coinlore" } });
      }
    ...
    const j = await r.json();
    return NextResponse.json(j, { headers: { "Cache-Control": "public, s-maxage=60, stale-while-revalidate=120", "X-CP-Source": "coingecko" } });
  } catch (e: unknown) {
