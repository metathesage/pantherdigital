    ...
    <div className="radar-app min-h-screen bg-[#F8F8F7] text-[#0A0A0A]">
      ...
      <div className="mx-auto grid max-w-[1600px] grid-cols-12 gap-4 px-4 py-4 sm:gap-5 sm:px-6 sm:py-5">
          ...
          <div className="mt-6">
            ...
            {sorted.length===0&&!loading?<div className="card grid place-items-center py-16 text-[#6B6B6B]">No signals match your filters.</div>:(
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 2xl:grid-cols-3">
                {loading?Array.from({length:6}).map((_,i)=><div key={i} className="card p-4" aria-label="Loading signal"><div className="flex items-start justify-between"><div className="flex items-center gap-3"><div className="size-11 rounded-xl bg-[#E8E8E8] animate-pulse"/><div className="flex flex-col gap-2"><div className="h-3.5 w-28 rounded-full bg-[#E8E8E8] animate-pulse"/><div className="h-3 w-20 rounded-full bg-[#F2F2F2] animate-pulse"/></div></div><div className="size-[52px] rounded-full border-4 border-[#F2F2F2] border-t-[#D1D1D1] animate-spin"/></div><div className="mt-5 h-5 w-32 rounded-full bg-[#E8E8E8] animate-pulse"/><div className="mt-4 grid grid-cols-2 gap-2"><div className="h-14 rounded-xl bg-[#F2F2F2] animate-pulse"/><div className="h-14 rounded-xl bg-[#F2F2F2] animate-pulse"/></div><div className="mt-4 h-10 rounded-xl bg-[#F2F2F2] animate-pulse"/></div>):sorted.slice(0,showCount).map(coin=>{
        ...
        <div className="col-span-12 space-y-4 xl:col-span-4 2xl:col-span-3">
          <div className="card overflow-hidden p-0"><div className="bg-[#0A0A0A] p-4 text-white sm:p-5"><div className="flex items-center justify-between"><h3 className="flex items-center gap-1.5 text-[11px] font-bold tracking-[0.14em]"><IconOrbit className="size-3.5"/> RADAR RANKINGS</h3><span className="rounded-full border border-white/20 px-2 py-1 text-[10px] font-semibold text-white/70">TOP 6 SIGNALS</span></div><p className="mt-2 text-[13px] text-white/60">The clearest momentum, attention, and liquidity signals right now.</p></div><div className="p-4 sm:p-5"><div className="mb-3 flex items-center justify-between text-[11px] text-[#6B6B6B]"><span>Sorted by emergent score</span><span>{loading?"Syncing feed…":"Live market feed"}</span></div><div className="space-y-2">{loading?Array.from({length:4}).map((_,i)=><div key={i} className="flex items-center gap-3 rounded-xl border border-[#E8E8E8] p-3"><div className="size-7 rounded-full bg-[#F2F2F2] animate-pulse"/><div className="size-8 rounded-lg bg-[#F2F2F2] animate-pulse"/><div className="flex-1"><div className="h-3 w-20 rounded-full bg-[#E8E8E8] animate-pulse"/><div className="mt-2 h-2.5 w-28 rounded-full bg-[#F2F2F2] animate-pulse"/></div><div className="h-5 w-8 rounded-full bg-[#E8E8E8] animate-pulse"/></div>):radarSorted.map((c,idx)=>(<button key={c.id} onClick={()=>setSelected(c)} className={`rank-row flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left ${selected?.id===c.id?"border-[#0A0A0A] bg-[#F8F8F7]":"border-[#E8E8E8] bg-white hover:border-[#0A0A0A]"}`}><span className={`grid size-7 place-items-center rounded-full border text-[12px] font-bold ${idx===0?"bg-[#0A0A0A] text-white border-[#0A0A0A]":"border-[#E8E8E8] bg-white"}`}>{idx+1}</span><img src={c.image} alt={c.symbol} className="size-8 rounded-lg border border-[#E8E8E8] bg-white object-cover"/><span className="flex-1"><span className="block text-[14px] font-semibold leading-none">{c.symbol}</span><span className="block text-[12px] text-[#6B6B6B]">{c.chain} · {c.trend}</span></span><span className="text-right"><span className="block text-[17px] font-bold tabular-nums tracking-tight">{c.emergentScore}</span><span className="block text-[11px]">{c.change24h>=0?"+":""}{c.change24h.toFixed(1)}%</span></span></button>))}</div></div></div>
