  ...
  return (
    <div className="radar-app min-h-screen bg-[#F8F8F7] text-[#0A0A0A]">
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="hidden items-center gap-2 text-[12px] tracking-wide text-[#6B6B6B] sm:flex"><span className="inline-flex items-center gap-1.5" aria-hidden="true"><span className="size-1.5 rounded-full bg-[#0A0A0A] animate-[pulse-dot_1.6s_ease-in-out_infinite]"/></span><span className="text-[#0A0A0A] font-medium tabular-nums">{coins.length?`${coins.length} coins`:"loading…"}</span>{lastUpdated&&<span className="text-[#9A9A9A]">· {lastUpdated.toLocaleTimeString()}</span>}<button onClick={()=>fetchCoins()} className="ml-1 rounded-full border border-[#E8E8E8] bg-white px-2 py-0.5 text-[11px] font-semibold hover:border-[#0A0A0A]">Refresh</button></div>
            </div>
          </div>
                ...
                {loading?Array.from({length:6}).map((_,i)=><div key={i} className="card animate-pulse p-4"><div className="h-11 w-11 rounded-xl bg-[#E8E8E8]"/><div className="mt-4 h-4 w-2/3 bg-[#E8E8E8] rounded"/></div>):sorted.slice(0,60).map(coin=>{
                  ...
                  return (
                    <button type="button" key={coin.id} onClick={()=>setSelected(coin)} className={`card card-hover coin-card flex flex-col p-3 text-left sm:p-3.5 ${surging?"surge-glow surge-ring":""} ${is90?"surge-90":""}`}>
                      ...
                      <div className="mt-3 flex items-center gap-2"><button type="button" aria-label="Toggle watchlist" onClick={(e)=>{e.stopPropagation(); setWatchlist(prev=>{const n=new Set(prev); n.has(coin.id)?n.delete(coin.id):n.add(coin.id); return n;});}} className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-full border px-3 py-2.5 text-[13px] font-semibold ${isWatched?"border-[#0A0A0A] bg-[#0A0A0A] text-white":"border-[#0A0A0A] bg-white hover:bg-[#F8F8F7]"}`}><IconStar className={`size-4 ${isWatched?"fill-white":""}`}/> {isWatched?"Watching":"Watchlist"}</button><button type="button" aria-label="Toggle alert" onClick={(e)=>{e.stopPropagation(); setAlerts(prev=>{const n=new Set(prev); n.has(coin.id)?n.delete(coin.id):n.add(coin.id); return n;});}} className={`grid size-11 place-items-center rounded-full border ${hasAlert?"border-[#0A0A0A] bg-[#0A0A0A] text-white":"border-[#E8E8E8] bg-white"}`}><IconBell className="size-4"/></button></div>
                    </button>
                  );
                })}
              </div>
      ...
      <div className={`fixed inset-0 z-50 ${selected?"visible":"invisible"}`}>
        <div onClick={()=>setSelected(null)} className={`absolute inset-0 bg-[#0A0A0A]/25 backdrop-blur-[2px] transition ${selected?"opacity-100":"opacity-0"}`}/>
        <div role="dialog" aria-modal="true" aria-label="Coin details" className={`detail-drawer absolute right-0 top-0 h-full w-full max-w-[560px] overflow-y-auto border-l border-[#E8E8E8] bg-white transition duration-300 ${selected?"translate-x-0":"translate-x-full"}`}>
          {selected&&(
            <div className="p-6">
