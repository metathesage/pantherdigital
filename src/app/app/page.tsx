...
// ---- verified chain resolution (no guessing) ----
// PLATFORMS: coin id -> CoinGecko asset-platform keys, loaded once from /api/coins/platforms
...
async function loadPlatforms(): Promise<void> {
  ...
  try {
    const r = await fetch("/api/coins/platforms", { cache: "force-cache" });
    if (!r.ok) return;
    const list: { id: string; platforms: Record<string, string> }[] = await r.json();
    const map: Record<string, string[]> = {};
    for (const c of list) {
      const keys = Object.keys(c.platforms || {}).filter((k) => c.platforms[k]);
      if (keys.length) map[c.id] = keys;
    }
    PLATFORMS = map;
...
const NATIVE_CHAIN: Record<string, Chain> = {
  ...
  ripple: "Multi", cardano: "Multi", dogecoin: "Multi", tron: "Multi", litecoin: "Multi",
  "bitcoin-cash": "Multi", stellar: "Multi", hedera: "Multi", algorand: "Multi",
  polkadot: "Multi", cosmos: "Multi", avalanche: "Multi", toncoin: "Multi",
  "near-protocol": "Multi", arbitrum: "Multi", optimism: "Multi", sei: "Multi",
  injective: "Multi", kaspa: "Multi", celestia: "Multi", aptos: "Multi",
};
...
function chainForCoin(c: GeckoCoin): Chain {
  ...
  const plats = (PLATFORMS && PLATFORMS[id]) || [];
  if (plats.includes("solana")) return "Solana";
  if (plats.includes("base")) return "Base";
  if (plats.includes("sui")) return "Sui";
  if (plats.includes("ethereum")) return "Ethereum";
  // 4) Token on another chain (L2 / alt L1) or platform data missing → Multi (honest bucket)
  return "Multi";
}
function platformsForCoin(id: string): string[] {
  return (PLATFORMS && PLATFORMS[id]) || [];
}
...
function getHoneypotUrl(coin: Coin, detail: any): string | null {
  ...
  if (coin.chain === "Solana") {
    ...
    const chainKey = coin.chain === "Ethereum" ? "ethereum" : coin.chain === "Base" ? "base" : coin.chain === "Sui" ? "sui" : "ethereum";
    const contract = platforms[chainKey];
    if (contract) {
...
export default function EmergentMinimal(){
  ...
  // never hang on Initializing Privy — demo fallback forces ready after 2.5s; also covers invalid appId
  const [privyReadyOverride, setPrivyReadyOverride] = useState(false);
  useEffect(()=>{ if(ready) return; const t=setTimeout(()=>setPrivyReadyOverride(true), 2500); return()=>clearTimeout(t); },[ready]);
  const effectiveReady = ready || privyReadyOverride;
  ...
  const connectMetaMask=async()=>{
    ...
    try{
      ...
      const msg=`Sign in to PNTHR DGTL — Emergent Matrix\n\nWelcome! Sign this message to verify you own this wallet and create your PNTHR DGTL profile.\n\nNonce: ${nonce}\nTime: ${new Date().toISOString()}`;
  ...
  const connectPhantom=async()=>{
    ...
    try{
      ...
      const msg=new TextEncoder().encode(`Sign in to PNTHR DGTL — Emergent Matrix\nNonce: ${Math.random().toString(36).slice(2,8).toUpperCase()}\nTime: ${new Date().toISOString()}`);
  ...
  useEffect(()=>{ fetchCoins(); const id=setInterval(fetchCoins,180000); return()=>clearInterval(id); },[]);
  ...
  if(!effectiveReady) return <div className="grid min-h-screen place-items-center bg-[#F8F8F7] text-[14px] text-[#6B6B6B]">Loading…</div>;
  return (
    <div id="main" className="radar-app min-h-screen bg-[#F8F8F7] text-[#0A0A0A]">
      ...
      <header className="sticky top-0 z-40 border-b border-[#E8E8E8] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            <Link href="/" className="grid size-11 place-items-center rounded-2xl border border-[#0A0A0A] bg-white overflow-hidden p-0.5 hover:bg-[#F8F8F7]"><img src="/panther-icon.png" alt="PNTHR DGTL" className="h-10 w-10 object-contain"/></Link>
            <div>
              ...
              <div className="hidden items-center gap-2 text-[12px] tracking-wide text-[#6B6B6B] sm:flex"><span className="inline-flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-[#0A0A0A]"/> Live</span><span className="text-[#0A0A0A] font-medium">{coins.length?`${coins.length} coins`:"loading…"}</span>{lastUpdated&&<span className="text-[#6B6B6B]">· {lastUpdated.toLocaleTimeString()}</span>}<button onClick={()=>fetchCoins()} className="ml-1 rounded-full border border-[#E8E8E8] bg-white px-2 py-0.5 text-[11px] font-semibold hover:border-[#0A0A0A]">Refresh</button></div>
            </div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Link href="/portfolio" title="Portfolio · Wallets" aria-label="Portfolio" className="grid size-9 place-items-center rounded-full border border-[#E8E8E8] bg-white hover:border-[#0A0A0A] hover:bg-[#0A0A0A] hover:text-white transition-colors sm:size-10"><IconWallet className="size-4"/></Link>
            <Link href="/about" title="Wiki · About" aria-label="About" className="grid size-9 place-items-center rounded-full border border-[#E8E8E8] bg-white hover:border-[#0A0A0A] hover:bg-[#0A0A0A] hover:text-white transition-colors sm:size-10"><IconGlobe className="size-4"/></Link>
            <Link href="/bio/rias" title="Waifu Squad" className="grid size-9 place-items-center rounded-full border border-[#E8E8E8] bg-white hover:border-[#e546a6] hover:bg-white transition-colors sm:size-10" aria-label="Waifu squad"><img src="/rias-waifu.png" alt="waifu" className="h-6 w-6 object-contain rounded-full"/></Link>
            {isConnected ? (
          ...
          {tickerHover && (
            <div className="absolute left-1/2 top-full z-20 mt-2 hidden -translate-x-1/2 rounded-2xl border border-[#0A0A0A] bg-white p-3 shadow-2xl sm:flex gap-3 min-w-[380px]">
              ...
              <div className="flex-1">
                ...
                <div className="mt-1 flex gap-2 text-[11px]"><span className="font-mono font-semibold">{tickerHover.price}</span><span className="text-[#6B6B6B]">· Vol {tickerHover.volume} · Score {tickerHover.emergentScore} · {tickerHover.trend}</span></div>
        ...
        <div className="col-span-12 xl:col-span-8 2xl:col-span-9">
          <div className="card p-3 sm:p-4">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <span className="ml-auto hidden items-center gap-1.5 text-[11px] text-[#6B6B6B] sm:inline-flex"><img src="/assets/mapped/rwa.png" alt="" className="size-4 rounded-full bg-white object-contain border border-[#E8E8E8]"/> {filtered.length} in bucket</span>
            </div>
            <div className="mt-5 grid grid-cols-12 gap-3">
              <div className="col-span-12 sm:col-span-8"><div className="relative"><IconSearch className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-[#6B6B6B]"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search CoinGecko / CoinMarketCap — real names, try bitcoin, solana, pepe…" className="h-[52px] w-full rounded-full border border-[#0A0A0A] bg-white py-3 pl-11 pr-4 text-[15px] font-medium placeholder:text-[#6B6B6B] focus:outline-none focus:ring-2 focus:ring-[#0A0A0A]/10"/><span className="pointer-events-none absolute right-2 top-1/2 hidden -translate-y-1/2 items-center gap-1 rounded-full bg-[#F2F2F2] px-3 py-1.5 text-[11px] font-semibold tracking-wide text-[#6B6B6B] sm:flex"><IconSearch className="size-3.5"/> CMC · CGK</span></div></div>
          ...
          <div id="pulse" className="card mt-5 overflow-hidden scroll-mt-24">
            ...
            <div className="grid grid-cols-1 gap-3 p-3 sm:grid-cols-12">
              ...
              <div className="sm:col-span-5 rounded-xl border border-[#E8E8E8] bg-white p-3">
                <div className="flex items-center justify-between"><span className="text-[11px] font-bold tracking-wide text-[#6B6B6B] flex items-center gap-1"><IconVol className="size-3"/> TOP 3 VOLUME · TODAY</span><span className="text-[10px] text-[#6B6B6B]">24h volume leaders</span></div>
              ...
              <div className="sm:col-span-3 rounded-xl border border-[#E8E8E8] bg-white p-3">
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-1 flex justify-between text-[10px] text-[#6B6B6B]"><span>0 Fear</span><span>100 Greed</span></div>
                </div>
                ...
                {fng?.history && fng.history.length>1 && (
                  <div className="mt-2 flex gap-1">{fng.history.slice(0,7).reverse().map((d:any,i:number)=>(<div key={i} className="flex-1 text-center"><div className="mx-auto h-8 w-full rounded bg-[#F2F2F2] relative overflow-hidden"><div className="absolute bottom-0 w-full bg-[#0A0A0A]" style={{height:`${d.value}%`}}/></div><div className="mt-1 text-[9px] text-[#6B6B6B]">{d.value}</div></div>))}</div>
                )}
                <div className="mt-2 text-[10px] text-[#6B6B6B]">Source: <a href="https://alternative.me/crypto/fear-and-greed-index/" target="_blank" rel="noreferrer" className="underline">alternative.me</a> + CoinGecko global · <a href="/api/fng" target="_blank" rel="noreferrer" className="underline">/api/fng</a></div>
              </div>
            </div>
          </div>
          {/* TRENDING NEWS — just below market pulse */}
          <div id="news" className="card mt-4 p-4 scroll-mt-24">
          ...
          <div id="spotlight" className="card mt-4 p-4 border-[#0A0A0A] scroll-mt-24">
          ...
          <div id="coins" className="mt-6 scroll-mt-24">
          ...
          <div className="card mt-6 p-4 border-[#0A0A0A]">
                ...
                {xScans.slice(0,8).map((x:any,i:number)=>(
                  <a key={i} href={x.url} target="_blank" rel="noreferrer" className="flex gap-3 rounded-xl border border-[#E8E8E8] bg-white p-3 hover:border-[#0A0A0A] hover:bg-[#F8F8F7] text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-[11px]"><span className="font-mono font-bold">{x.mentions} mentions</span><span className="text-[#6B6B6B]">· {new Date(x.ts).toLocaleTimeString()} · {x.platform}</span></div>
                    </div>
            ...
            <div className="mt-2 text-[11px] text-[#6B6B6B]">Sources: <a href="https://api.dexscreener.com/token-boosts/top/v1" target="_blank" rel="noreferrer" className="underline">DexScreener boosts</a> (KOL paid) + <a href="https://www.coingecko.com/en/search/trending" target="_blank" rel="noreferrer" className="underline">CoinGecko trending</a> · <a href="/api/x-scan" target="_blank" rel="noreferrer" className="underline">/api/x-scan</a> · no X API key needed</div>
          </div>
          ...
          <div className="mt-6">
            ...
            {activeExtra==='nfts' ? (
              <div className="mt-3">
                ...
                <div className="mt-2 text-[11px] text-[#6B6B6B]">Source: <a href="https://www.coingecko.com/en/nfts" target="_blank" rel="noreferrer" className="underline">CoinGecko NFTs</a> · cryptoslam-style — top sold in {nftTimeframe}, rich images, real links (OpenSea/Blur/CGK). Floor/vol live.</div>
              </div>
            ) : (
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {rwaCoins.length===0 ? <div className="card grid place-items-center py-10 text-sm text-[#6B6B6B] col-span-2">No RWA coins in current set — switch bucket to RWA to see all.</div> : rwaCoins.map(c=>(
                  <button key={c.id} onClick={()=>setSelected(c)} className="card card-hover flex gap-3 p-3 text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex gap-3 text-[11px] font-mono"><span className="font-bold">{c.price}</span><span className={c.change24h>=0?"text-emerald-600":"text-red-600"}>{c.change24h>=0?"+":""}{c.change24h.toFixed(2)}%</span><span className="text-[#6B6B6B]">Vol {c.volume} · Cap {c.marketCap}</span></div>
                    </div>
      ...
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 pb-10 space-y-6">
        ...
        <div id="trending-news-full" className="card p-5">
          ...
          <div className="mt-3 text-[11px] text-[#6B6B6B]">Sources: CoinDesk, CoinTelegraph, Decrypt via RSS · <a href="/api/news" target="_blank" rel="noreferrer" className="underline">/api/news</a> cached 60s</div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="card p-4 lg:col-span-1">
            ...
            <div className="mt-2 text-[11px] text-[#6B6B6B]">Source: <a href="https://api.dexscreener.com/token-boosts/latest/v1" target="_blank" rel="noreferrer" className="underline">api.dexscreener.com</a> · no key</div>
          </div>
                  ...
                  <div className="mt-2 space-y-1.5">
                    {Object.entries(detail.platforms as Record<string,string>).slice(0,3).map(([chain, addr]: any)=>(
                      <div key={chain} className="flex items-center gap-2 rounded-xl border border-[#E8E8E8] bg-[#F8F8F7] px-3 py-2">
                        ...
                        <a href={chain==="ethereum"?`https://etherscan.io/address/${addr}`:chain==="base"?`https://basescan.org/address/${addr}`:chain==="solana"?`https://solscan.io/account/${addr}`:chain==="sui"?`https://suiscan.xyz/mainnet/object/${addr}`:`https://etherscan.io/address/${addr}`} target="_blank" rel="noreferrer" className="rounded-full bg-[#0A0A0A] px-2 py-1 text-[11px] font-semibold text-white">Explorer ↗</a>
                      </div>
                    ))}
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <div className="rounded-xl border border-[#E8E8E8] bg-white p-3">
                    ...
                    <div className="mt-2 space-y-1.5">
                      ...
                      {coins.filter(c=>c.category===selected.category).length===0 && <div className="text-[11px] text-[#6B6B6B]">No related in {selected.category} — try Meme/Robinhood.</div>}
                    </div>
                    <div className="mt-2 flex gap-1.5">
      ...
      {showConnect&&(<div className="fixed inset-0 z-[60] grid place-items-center bg-black/85 p-4 backdrop-blur-sm"><div className="relative w-full max-w-[440px] overflow-hidden rounded-[28px] border border-white/15 bg-black shadow-2xl"><img src="/black-marble-panther.jpg" alt="" aria-hidden className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-[0.38]" /><div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/40 via-black/55 to-black/90" /><div className="relative p-6"><div className="flex items-start justify-between"><div><h2 className="text-[20px] font-bold flex items-center gap-2 text-white"><img src="/panther-icon.png" alt="" className="h-6 w-6 object-contain drop-shadow" /> Connect to PNTHR DGTL</h2><p className="mt-1 text-[14px] leading-6 text-white/65">Black marble, panther precision — pick your rail.</p></div><button onClick={()=>setShowConnect(false)} className="grid size-9 place-items-center rounded-full border border-white/15 bg-white/10 text-white hover:bg-white/20"><IconX className="size-4"/></button></div><div className="mt-6 space-y-3"><button onClick={()=>{login(); setShowConnect(false);}} className="flex w-full items-center gap-3 rounded-2xl border border-white/15 bg-white/95 px-4 py-4 text-left backdrop-blur hover:bg-white"><span className="grid size-11 place-items-center rounded-xl bg-black text-white"><IconWallet className="size-5 text-white"/></span><span className="flex-1"><span className="block text-[15px] font-semibold text-black">Privy — Wallet / Email</span><span className="block text-[13px] text-black/60">Real Privy auth</span></span><span className="rounded-full bg-black px-3 py-1 text-[12px] font-semibold text-white">Real</span></button><button onClick={()=>{login(); setShowConnect(false);}} className="flex w-full items-center gap-3 rounded-2xl border border-white/15 bg-white/90 px-4 py-4 text-left backdrop-blur hover:bg-white"><span className="grid size-11 place-items-center rounded-xl border border-black/10 bg-white">𝕏</span><span className="flex-1"><span className="block text-[15px] font-semibold text-black">Continue with X</span><span className="block text-[13px] text-black/60">Via Privy Twitter OAuth</span></span><span className="text-black/40">→</span></button>
              ...
              <button onClick={connectCoinbase} className="flex w-full items-center gap-3 rounded-2xl border border-white/15 bg-white/90 px-4 py-4 text-left backdrop-blur hover:bg-white"><span className="grid size-11 place-items-center rounded-xl bg-[#0052FF] text-[11px] font-black text-white">CB</span><span className="flex-1"><span className="block text-[15px] font-semibold text-black">Coinbase Wallet</span><span className="block text-[13px] text-black/60">Base / EVM — one tap</span></span><span className="text-black text-[12px] font-semibold">Connect →</span></button>
      ...
      <section id="about" className="mx-auto max-w-[1600px] px-4 pt-6 sm:px-6">
        <div className="card p-6">
          <h2 className="text-[14px] font-black tracking-[0.14em] flex items-center gap-2"><IconGlobe className="size-4"/> ABOUT — PNTHR DGTL</h2>
          <p className="mt-2 text-[14px] leading-6 text-[#1A1A1A]">PNTHR DGTL is a minimal, luxury discovery engine for 1000+ real coins. We pull live prices, images, spark lines, tickers and socials directly from <a href="https://www.coingecko.com" target="_blank" rel="noreferrer" className="underline">CoinGecko</a> (no key), with trading pairs via <a href="https://dexscreener.com" target="_blank" rel="noreferrer" className="underline">Dexscreener</a> (Robinhood Chain: <a href="https://dexscreener.com/robinhood" target="_blank" rel="noreferrer" className="underline">dexscreener.com/robinhood</a>), charts via <a href="https://www.coingecko.com/api" target="_blank" rel="noreferrer" className="underline">CoinGecko market_chart</a>, and wallet auth via <a href="https://www.privy.io" target="_blank" rel="noreferrer" className="underline">Privy</a> + free injected wallets (MetaMask/Phantom/Coinbase — no API). Every contract is screened for honeypots before listing — SOL via <a href="https://rugcheck.xyz" target="_blank" rel="noreferrer" className="underline">RugCheck.xyz</a>, EVM via <a href="https://gopluslabs.io" target="_blank" rel="noreferrer" className="underline">GoPlus</a> (trust_score). Only verified non-honeypot assets are shown.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3 text-[13px]">
            <div className="rounded-xl border border-[#E8E8E8] bg-[#F8F8F7] p-3"><div className="font-semibold">Sources</div><div className="mt-1 text-[#6B6B6B]">CoinGecko (prices/images), Dexscreener (pairs, Robinhood Chain), CoinMarketCap (links), pump.fun, X, Birdeye/Jupiter (SOL)</div></div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[12px]">
            ...
            <a href="https://dexscreener.com/robinhood" target="_blank" rel="noreferrer" className="rounded-full border border-[#E8E8E8] bg-white px-3 py-1.5">Dexscreener ↗</a>
