      ...
      <header className="sticky top-0 z-40 border-b border-[#E8E8E8] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-baseline gap-2"><span className="text-[18px] font-bold tracking-[0.14em]">COIN</span><span className="text-[18px] font-light tracking-[0.18em] text-[#6B6B6B]">PANTHER</span><span className="ml-1 hidden rounded-full border border-[#0A0A0A] px-2 py-0.5 text-[10px] font-semibold tracking-widest sm:inline-block">LIVE</span><span className="ml-1 hidden items-center gap-1 rounded-full bg-[#FF6B00] px-2 py-0.5 text-[10px] font-bold tracking-widest text-white sm:inline-flex hallow-glow"><img src="/assets/halloween/cryptohallow_btc_pumpkin_1024.png" alt="" className="size-3.5 rounded-full bg-white object-cover hallow-float"/> SPOOKY SEASON</span></div>
      ...
      <div className="mx-auto grid max-w-[1600px] grid-cols-12 gap-5 px-4 py-6 sm:px-6">
        <div className="col-span-12 xl:col-span-8 2xl:col-span-9">
          <div className="card p-4 sm:p-5">
            <div className="flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] text-[#FF6B00]"><img src="/assets/halloween/cryptohallow_sol_ghost_1024.png" alt="" className="size-5 rounded-full bg-white object-cover border border-[#FF6B00]/30"/> <span>🦇 HAUNTED MARKETS — HALLOWEEN EDITION</span><span className="ml-auto text-[11px] font-semibold normal-case tracking-normal text-[#6B6B6B]">Ghost chains & pumpkin pumps</span></div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {CHAINS.map(ch=>{ const active=chainFilter===ch; const count=ch==="All"?coins.length:coins.filter(c=>c.chain===ch).length; const hIcon = ch==="Solana"?"/assets/halloween/cryptohallow_sol_ghost_1024.png": ch==="Ethereum"?"/assets/halloween/cryptohallow_eth_batwings_1024.png": null; return (<button key={ch} onClick={()=>setChainFilter(ch)} className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[14px] font-semibold transition ${active?"bg-[#0A0A0A] text-white":"border border-[#E8E8E8] bg-white text-[#0A0A0A] hover:border-[#0A0A0A]"}`}>{hIcon && <img src={hIcon} alt="" className="size-4 rounded-full bg-white object-cover"/>}{ch==="All"?"All":ch} <span className="ml-1 text-[11px] text-[#6B6B6B]">{count}</span></button>); })}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {BUCKETS.map(b=>{ const active=bucketFilter===b; const count=b==="All"?coins.length:coins.filter(c=>c.category===b).length; const hIcon = b==="DeFi"?"/assets/halloween/cryptohallow2_defi_haunted_1024.png" : b==="Meme"?"/assets/halloween/cryptohallow2_meme_witch_1024.png" : b==="Stable"?"/assets/halloween/cryptohallow_btc_pumpkin_1024.png" : null; return (<button key={b} onClick={()=>setBucketFilter(b)} className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[13px] font-semibold ${active?"bg-[#0A0A0A] text-white":"border border-[#E8E8E8] bg-white hover:border-[#0A0A0A]"}`}>{hIcon && <img src={hIcon} alt="" className="size-4 rounded-full bg-white object-cover"/>}{b} <span className="ml-1 text-[10px] text-[#6B6B6B]">{count}</span></button>); })}
              <span className="ml-auto hidden items-center gap-1.5 text-[11px] text-[#9A9A9A] sm:inline-flex"><img src="/assets/halloween/cryptohallow2_nft_spider_1024.png" alt="" className="size-4 rounded-full bg-white object-cover"/> {filtered.length} in bucket</span>
            </div>
