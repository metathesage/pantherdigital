...
function chainForCoin(c: GeckoCoin): Chain {
  ...
  // explicit Robinhood chain — CashCat + other Robinhood-native
  if(s==="cashcat"||id==="cashcat"||id.includes("cashcat")||s==="hood"||id.includes("robinhood")) return "Robinhood";
  ...
  // Robinhood badge for top listed — but keep native chain; only CashCat above forces Robinhood
  // deterministic fallback across all chains
  const mods: Chain[] = ["Solana","Ethereum","Base","Sui","Robinhood"];
  if (["btc","eth","sol","avax","ada","apt","near","atom","sei","tia","inj","kas"].includes(s)) return mods[c.market_cap_rank % mods.length];
