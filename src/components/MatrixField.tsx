...
export default function MatrixField() {
  ...
  useEffect(() => {
    ...
    if (!host) return;
    let renderer: { dispose: () => void; domElement: HTMLCanvasElement; setSize: (w: number, h: number, update: boolean) => void; render: (scene: unknown, camera: unknown) => void } | null = null;
    let raf = 0;
    ...
    (async () => {
      try {
        ...
        const wire = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: 0xfbbf24, wireframe: true, transparent: true, opacity: 0.16 })
        );
        group.add(wire);
        ...
        renderer = {
          ...
          setSize: (w, h, update) => webgl.setSize(w, h, update),
          render: (s, c) => webgl.render(s as import("three").Scene, c as import("three").Camera),
        };
