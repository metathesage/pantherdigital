...
export default function AlinaAvatar({
  mode = "auto",
  height = 560,
  className = "",
}: {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        ...
        console.info("[AlinaAvatar] VRM not available, using 2D fallback:", (e as Error)?.message ?? e);
        setStatus("fallback-2d");
    ...
    >
        ...
        <div className="absolute inset-0">
          ...
          {sprites.map((src, i) => (
            <img
              key={src}
              src={src}
              alt={i === 0 ? "Alina — Rias inspired" : "Alina reference"}
          ...
          <div className="absolute bottom-0 w-full p-4 sm:p-5">
            <div className="rounded-2xl bg-white/92 p-4 backdrop-blur border border-white/60 shadow-[0_12px_40px_rgba(0,0,0,0.14)]">
              <div className="text-[11px] font-bold tracking-[0.18em] text-[#6B6B6B]">ALINA · BOSS WAIFU</div>
      ...
      {status === "vrm" && (
        <div className="absolute bottom-3 left-3 rounded-2xl bg-white/90 px-3 py-2 text-[11px] leading-4 shadow backdrop-blur border border-white">
          <span className="font-bold tracking-widest text-[#0A0A0A]">ALINA · VRM 1.0</span>
